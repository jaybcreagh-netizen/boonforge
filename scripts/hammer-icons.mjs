...
const titlesFor = (suffix) => all.map((h) => 'File:' + h.name.replace(/_/g, ' ') + suffix)
for (const suffix of ['_II.png', '.png']) {
  for (let batch = 0; batch < Math.ceil(all.length / 50); batch++) {
    const slice = titlesFor(suffix).slice(batch * 50, (batch + 1) * 50)
    ...
    for (const p of json.query.pages) {
      if (p.imageinfo && !byTitle.has(p.title)) byTitle.set(p.title, p.imageinfo[0].thumburl || p.imageinfo[0].url)
    }
  }
}
...
for (const h of all) {
  const url =
    byTitle.get('File:' + h.name + '_II.png') ||
    byTitle.get('File:' + h.name + '.png')
  if (!url) {
    missing.push(h.id)
    continue
  }
...
console.log('icons downloaded:', ok, '/', all.length, '| missing:', missing.slice(0, 20).join(', ') || 'none')
